...
async function DOM_login_check() {
	try {
		...
		const response = await fetch("http://localhost:3000/DOM_login_check", {
			method: "POST",
...
form.addEventListener("submit", async function (event) {
	...
	try {
		const response = await fetch("http://localhost:3000/in", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
...
async function loginTrue_PUT(id1, login_value) {
	const response2 = await fetch("http://localhost:3000/loginTrue", {
		method: "PUT",
		headers: { "Content-Type": "application/json" },
